...
if [ ! -z "${ES_PLUGINS_INSTALL}" ]; then
    ...
    for plugin in ${ES_PLUGINS_INSTALL}; do
        ...
        if ! "${ES_BASE}"/bin/elasticsearch-plugin list | grep -qs ${plugin}; then
            until "${ES_BASE}"/bin/elasticsearch-plugin install --batch ${plugin}; do
...
if [ ! -z "${AZURE_REPOSITORY_CONFIG}" ]; then
    if [ ! -z "${AZURE_REPOSITORY_ACCOUNT_NAME}" ] && [ ! -z "${AZURE_REPOSITORY_ACCOUNT_KEY}" ] ; then
        ...
        yes | "${ES_BASE}"/bin/elasticsearch-plugin install https://artifacts.elastic.co/downloads/elasticsearch-plugins/repository-azure/repository-azure-${ES_VERSION}.zip

        echo "${AZURE_REPOSITORY_ACCOUNT_NAME}" | "${ES_BASE}"/bin/elasticsearch-keystore add azure.client.default.account -f
        echo "${AZURE_REPOSITORY_ACCOUNT_KEY}" | "${ES_BASE}"/bin/elasticsearch-keystore add azure.client.default.key -f
    else
    ...
    if [ -f "${GCP_SA_JSON_CRED}" ] ; then
        echo "Configuring plugin : repository-gcs for ES version ${ES_VERSION}"
        yes | "${ES_BASE}"/bin/elasticsearch-plugin install https://artifacts.elastic.co/downloads/elasticsearch-plugins/repository-gcs/repository-gcs-${ES_VERSION}.zip

        bin/elasticsearch-keystore add-file gcs.client.default.credentials_file ${GCP_SA_JSON_CRED}
    else
        echo "GCS_REPOSITORY_CONFIG is there but ${GCP_SA_JSON_CRED} is missing..!"
        exit 1
    fi
elif [ ! -z "${S3_REPOSITORY_CONFIG}" ]; then
    if [ ! -z "${S3_ACCESS_KEY}" ] && [ ! -z "${S3_SECRET_KEY}" ] ; then
        echo "Configuring plugin : repository-s3 for ES version ${ES_VERSION}"
        yes | "${ES_BASE}"/bin/elasticsearch-plugin install https://artifacts.elastic.co/downloads/elasticsearch-plugins/repository-s3/repository-s3-${ES_VERSION}.zip

        echo "${S3_ACCESS_KEY}" | "${ES_BASE}"/bin/elasticsearch-keystore add s3.client.default.access_key -f
        echo "${S3_SECRET_KEY}" | "${ES_BASE}"/bin/elasticsearch-keystore add s3.client.default.secret_key -f
    else
